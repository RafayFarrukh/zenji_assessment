...
- **Home** — full-bleed split hero, collection strip, brand manifesto, the full ten-piece
  grid, lifestyle section.
...
- Announcement marquee, mobile nav, newsletter capture (validates in the browser; it is
  explicit that nothing is sent).
...
| **Tailwind v4**            | The design tokens live in one `@theme` block in `globals.css`, so the palette and type scale are enforced by the class names themselves.                                                                                                              |
...
| **Framer Motion**          | Three places only: the hero reveal, the cart drawer, and nothing else.                                                                                                                                                                                |
...
`clsx`/`tailwind-merge` (a four-line `cn()`), and no carousel library.
...
- **Same brand, different designer.** The reference site's own conventions — `//` labels,
  `WORD_WORD` naming, neon-on-black — are not reused. The brand facts are: black-dominant,
  limited drops, samurai/anime language, Japanese accents, A$39.99 oversized tees, Melbourne.
...
- **One bold move.** The hero is an asymmetric split — copy on five columns, uncropped
  ...
  cannot repeat, which is literally what a drop with no restock is. Everything below the
  hero is disciplined: 8pt spacing, a 12-column grid, hairline rules.
...
- **Motion is rare so it means something.** One hero reveal, one drawer transition, one
  image swap on card hover. No scroll-triggered fade-ups. Everything collapses under
...
- **The LCP element is the hero photograph**, and it is deliberately excluded from the
  entrance animation so it paints as soon as the bytes land rather than waiting on
  JavaScript. The hero copy has a `<noscript>` fallback so it is readable even if the bundle
  never arrives.
- **~102kB of shared JS** (First Load), 118–155kB per route.
- **No console errors or warnings** on any route.
- **No horizontal overflow** at 360, 390, 768, 1440 or 1920px.
...
src/
  ...
  components/               flat, no over-abstraction; ui/ holds the primitives
...
client paint matches the server HTML and React never warns about a mismatch.
